...
| macOS single-elevation redirect — a persistent, self-terminating root helper applies/reverts the pf anchor, so a capture session needs one admin-password prompt instead of one per toggle; self-heals within ~15s if the extension host goes silent | ✅ | [network-inspector.md](./network-inspector.md#macos-single-elevation-persistent-helper) |
