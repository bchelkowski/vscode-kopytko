...
export interface SgNodeDefinition {
  name: string;
  extends?: string;
  description: string;
  docsUrl: string;
...
export const SG_NODES: Record<string, SgNodeDefinition> = {
  ...
  Audio: {
    ...
    fields: [
      { name: 'content', type: 'node', description: 'ContentNode with audio stream metadata or playlist.' },
      { name: 'contentIsPlaylist', type: 'boolean', description: 'When true, enables sequential playlist playback.' },
      { name: 'contentIndex', type: 'integer', description: 'Index of the currently playing track.', readonly: true },
      { name: 'nextContentIndex', type: 'integer', description: 'Write to set the next playlist track. -1 for auto-increment.' },
      { name: 'control', type: 'string', description: 'Write "play", "stop", "pause", "resume", "replay", "prebuffer", or "skipcontent".' },
      { name: 'state', type: 'string', description: '"none", "buffering", "playing", "paused", "stopped", "finished", or "error".', readonly: true },
      { name: 'position', type: 'float', description: 'Current playback position in seconds.', readonly: true },
      { name: 'duration', type: 'float', description: 'Total duration in seconds.', readonly: true },
      { name: 'seek', type: 'float', description: 'Write a position in seconds to seek.' },
      { name: 'loop', type: 'boolean', description: 'When true, playback restarts after finishing.' },
      { name: 'mute', type: 'boolean', description: 'When true, audio is muted.' },
      { name: 'autoplayAfterSeek', type: 'boolean', description: 'When true, playback automatically resumes after rebuffering.' },
      { name: 'notificationInterval', type: 'float', description: 'Frequency of position observer notifications in seconds; 0 disables.' },
      { name: 'timedMetaDataSelectionKeys', type: 'array', description: 'Keys to extract from timed metadata in the stream.' },
      { name: 'timedMetaData', type: 'assocarray', description: 'Most recently decoded timed metadata.', readonly: true },
      { name: 'bufferingStatus', type: 'assocarray', description: 'Buffering progress and status information.', readonly: true },
      { name: 'streamingSegment', type: 'assocarray', description: 'Data about the currently streaming segment (HLS/DASH).', readonly: true },
      { name: 'streamInfo', type: 'assocarray', description: 'Network and bitrate information for the active stream.', readonly: true },
      { name: 'timeToStartStreaming', type: 'float', description: 'Seconds elapsed before audio playback began.', readonly: true },
      { name: 'audioFormat', type: 'string', description: 'Format of the current stream (aac, mp3, flac, etc.).', readonly: true },
      { name: 'errorCode', type: 'integer', description: 'Numeric error code when state is "error".', readonly: true },
      { name: 'errorMsg', type: 'string', description: 'Human-readable error description.', readonly: true },
    ],
    methods: [],
  },
  ...
  ChannelStore: {
    ...
    fields: [
      { name: 'command', type: 'string', description: 'Command to execute: "getUserData", "doOrder", "getPurchases", "getCatalog", "getStoreCatalog", "getAllPurchases", "requestPartnerOrder", "confirmPartnerOrder", "getChannelCred", "getDeviceAttestationToken".' },
      { name: 'requestedUserData', type: 'string', description: 'Comma-separated Roku account fields to request. Default "all".' },
      { name: 'requestedUserDataInfo', type: 'node', description: 'Controls RFI screen behavior (sign-up vs sign-in context).' },
      { name: 'userData', type: 'node', description: 'Roku account data returned by getUserData.' },
      { name: 'order', type: 'node', description: 'ContentNode tree defining items to purchase.' },
      { name: 'deltaOrder', type: 'assocarray', description: 'Write-only: incremental order modification.' },
      { name: 'orderStatus', type: 'node', description: 'Result of a doOrder command.', readonly: true },
      { name: 'purchases', type: 'node', description: 'Results of getPurchases or getAllPurchases command.', readonly: true },
      { name: 'catalog', type: 'node', description: 'Available in-app products from getCatalog.', readonly: true },
      { name: 'storeCatalog', type: 'node', description: 'Globally available products from getStoreCatalog.', readonly: true },
      { name: 'requestPartnerOrderStatus', type: 'node', description: 'Result of requestPartnerOrder (TVOD).', readonly: true },
      { name: 'confirmPartnerOrderStatus', type: 'node', description: 'Result of confirmPartnerOrder.', readonly: true },
      { name: 'channelCredData', type: 'string', description: 'OAuth token or custom auth data stored in the Roku cloud.' },
      { name: 'channelCred', type: 'node', description: 'Result of getChannelCred command.', readonly: true },
      { name: 'nonce', type: 'string', description: 'Nonce for getDeviceAttestationToken JWT generation.' },
      { name: 'fakeServer', type: 'boolean', description: 'When true, uses local XML files instead of live server calls.' },
    ],
    methods: [],
  },
  ...
  Button: {
    ...
    fields: [
      ...
      { name: 'showFocusFootprint', type: 'boolean', description: 'When true, shows the focus footprint when unfocused.' },
      { name: 'focusFootprintBitmapUri', type: 'uri', description: 'Background bitmap shown when the button lacks focus.' },
      { name: 'focusBitmapUri', type: 'uri', description: 'Background bitmap shown when the button has focus.' },
      { name: 'iconUri', type: 'uri', description: 'Icon bitmap when the button lacks focus.' },
      { name: 'focusedIconUri', type: 'uri', description: 'Icon bitmap when the button has focus.' },
      ...
      { name: 'scrollable', type: 'boolean', description: 'When true, enables auto-scrolling of text that overflows the button.' },
      { name: 'buttonSelected', type: 'boolean', description: 'Toggles when the button is selected by the user.', readonly: true },
    ],
    methods: [],
  },
  ...
  Animation: {
    ...
    fields: [
      ...
      { name: 'easeFunction', type: 'string', description: '"linear", "inQuad", "outQuad", "inOutQuad", "inCubic", "outCubic", "inOutCubic", "piecewise", etc.' },
      { name: 'easeInPercent', type: 'float', description: 'Percentage of duration for ease-in (used when easeFunction is "piecewise").' },
      { name: 'easeOutPercent', type: 'float', description: 'Percentage of duration for ease-out (used when easeFunction is "piecewise").' },
      { name: 'optional', type: 'boolean', description: 'When true, the animation is skipped on lower-performing devices.' },
      { name: 'willBeSkipped', type: 'boolean', description: 'When true, the animation will jump to its end state instead of playing.', readonly: true },
    ],
    methods: [],
  },
  ...
  FloatFieldInterpolator: {
    ...
    fields: [
      ...
      { name: 'reverse', type: 'boolean', description: 'When true, the animation plays in reverse.' },
    ],
    methods: [],
  },
  ...
  ArrayGrid: {
    ...
    fields: [
      ...
      { name: 'fadeFocusFeedbackWhenAutoScrolling', type: 'boolean', description: 'When true, fades the focus indicator during multi-item auto-scroll.' },
      { name: 'currFocusFeedbackOpacity', type: 'float', description: 'Current opacity of the focus indicator.', readonly: true },
      { name: 'skipFocusAnimations', type: 'boolean', description: 'When true, disables focus movement animations.' },
      { name: 'itemClippingRect', type: 'array', description: '[x, y, width, height] clipping rectangle applied to each item.' },
      { name: 'currFocusSection', type: 'float', description: 'Section index of the focused item.', readonly: true },
      { name: 'rowSpacings', type: 'array', description: 'Array of per-row vertical spacings overriding itemSpacing.' },
      { name: 'sectionDividerBitmapUri', type: 'uri', description: 'Custom bitmap for section dividers.' },
      { name: 'sectionDividerHeight', type: 'float', description: 'Height of section dividers.' },
      { name: 'sectionDividerWidth', type: 'float', description: 'Width of section dividers.' },
      { name: 'sectionDividerMinWidth', type: 'float', description: 'Minimum width of section dividers.' },
      { name: 'sectionDividerLeftOffset', type: 'float', description: 'Left edge offset for section dividers.' },
      { name: 'sectionDividerSpacing', type: 'float', description: 'Spacing between section divider elements.' },
  ...
  LabelList: {
    ...
    fields: [
      ...
      { name: 'textHorizAlign', type: 'string', description: '"left", "right", or "center" — horizontal alignment of item text.' },
    ],
    methods: [],
  },
  ...
  RadioButtonList: {
    ...
    extends: 'LabelList',
    description: 'A LabelList where exactly one item is selected at a time (radio button behavior).',
    docsUrl: 'https://developer.roku.com/docs/references/scenegraph/list-and-grid-nodes/radiobuttonlist.md',
    fields: [
      { name: 'checkedItem', type: 'integer', description: 'Index of the currently selected item. -1 means none selected.' },
      { name: 'checkOnSelect', type: 'boolean', description: 'When true, pressing OK automatically updates checkedItem.' },
      { name: 'checkedIconUri', type: 'uri', description: 'Check icon for the selected item when unfocused.' },
      { name: 'focusedCheckedIconUri', type: 'uri', description: 'Check icon for the selected item when focused.' },
    ],
    methods: [],
  },
  ...
  PosterGrid: {
    ...
    fields: [
      ...
      { name: 'useAtlas', type: 'boolean', description: 'When true, uses a texture atlas for poster rendering (performance optimization).' },
      ...
      { name: 'captionBackgroundBitmapUri', type: 'uri', description: 'Background bitmap rendered behind caption text.' },
      { name: 'captionHorizAlignment', type: 'string', description: '"left", "center", or "right" — horizontal caption alignment.' },
      { name: 'captionVertAlignment', type: 'string', description: '"above", "top", "center", "bottom", or "below" — vertical caption alignment.' },
      { name: 'captionLineSpacing', type: 'float', description: 'Pixel spacing between caption lines.' },
      { name: 'showBackgroundForEmptyCaptions', type: 'boolean', description: 'When true, caption background is shown even when text is empty.' },
      { name: 'enableCaptionScrolling', type: 'boolean', description: 'When true, single-line captions scroll when ellipsized.' },
    ],
    methods: [],
  },
  ...
  ZoomRowList: {
    ...
    fields: [
      ...
      { name: 'itemClippingRect', type: 'array', description: 'Clipping rectangle controlling visible items.' },
      { name: 'rowItemYOffset', type: 'array', description: 'Vertical position of each row when unfocused.' },
      { name: 'rowItemZoomYOffset', type: 'array', description: 'Vertical position of each row when focused.' },
      { name: 'rowItemHeight', type: 'array', description: 'Item height when the row is unfocused.' },
      { name: 'rowItemZoomHeight', type: 'array', description: 'Item height when the row is focused.' },
      { name: 'rowItemAspectRatio', type: 'array', description: 'Aspect ratio for computing item width from height.' },
      { name: 'spacingAfterRowItem', type: 'array', description: 'Horizontal spacing between items; scales with zoom.' },
      { name: 'useDefaultAspectRatio', type: 'array', description: 'Per-row flag: use rowItemAspectRatio (true) or per-item values (false).' },
      ...
      { name: 'fadeFocusFeedbackWhenLongPressScrolling', type: 'boolean', description: 'Fades focus indicator during long-press scroll.' },
      { name: 'focusBitmapUri', type: 'uri', description: 'Focus indicator bitmap.' },
      { name: 'focusBitmapBlendColor', type: 'string', description: 'Tint for the focus bitmap.' },
      { name: 'wrapDividerBitmapUri', type: 'uri', description: 'Visual divider bitmap shown when wrapping.' },
      { name: 'wrapDividerBitmapBlendColor', type: 'string', description: 'Color blend for the wrap divider bitmap.' },
      { name: 'wrapDividerHeight', type: 'float', description: 'Height of the wrap divider.' },
      { name: 'wrapDividerWidth', type: 'float', description: 'Width of the wrap divider.' },
      { name: 'wrapDividerOffset', type: 'array', description: '[x, y] position adjustment for the wrap divider.' },
      { name: 'showRowTitle', type: 'array', description: 'Per-row: whether the row title label is shown.' },
      { name: 'rowTitleOffset', type: 'array', description: 'Per-row position offset for the row title.' },
      { name: 'rowTitleFont', type: 'node', description: 'Font for row title labels.' },
      { name: 'rowTitleColor', type: 'array', description: 'Per-row color for row title text.' },
      { name: 'showRowCounter', type: 'array', description: 'Per-row: whether the row counter is shown.' },
      { name: 'rowCounterOffset', type: 'array', description: 'Per-row position for the row counter (right-aligned).' },
      { name: 'rowCounterFont', type: 'node', description: 'Font for row counter labels.' },
      { name: 'rowCounterColor', type: 'array', description: 'Per-row color for row counter text.' },
      { name: 'showRowCounterForShortRows', type: 'boolean', description: 'When true, shows counter even for rows shorter than rowWidth.' },
      { name: 'rowDecorationComponentName', type: 'array', description: 'Per-row component names for custom row decorations.' },
      { name: 'rowSelected', type: 'integer', description: 'Index of the selected row.', readonly: true },
      { name: 'rowUnfocused', type: 'integer', description: 'Index of the row that lost focus.', readonly: true },
      ...
      { name: 'rowItemFocused', type: 'array', description: '[rowIndex, itemIndex] of the focused item.', readonly: true },
      { name: 'scrollingStatus', type: 'boolean', description: 'True while the list is actively scrolling.', readonly: true },
      { name: 'rowsRendered', type: 'array', description: '[firstRow, lastRow] indices currently rendered.', readonly: true },
      { name: 'rowItemsRendered', type: 'array', description: 'Rendered row and item index ranges.', readonly: true },
      { name: 'currFocusRow', type: 'float', description: 'Floating-point row index of current focus position.', readonly: true },
      { name: 'jumpToRow', type: 'integer', description: 'Write a row index to jump to that row.' },
      { name: 'jumpToRowItem', type: 'array', description: 'Write [rowIndex, itemIndex] to jump to that item.' },
      { name: 'animateToRow', type: 'integer', description: 'Animate-scroll to a row.' },
      { name: 'remainZoomedAboveFocus', type: 'string', description: 'Controls zoom behavior for rows above focus.' },
      { name: 'fadeOutAboveFocus', type: 'string', description: 'Controls fade behavior for rows above focus.' },
    ],
    methods: [],
  },
  ...
  TextEditBox: {
    ...
    docsUrl: 'https://developer.roku.com/docs/references/scenegraph/widget-nodes/texteditbox.md',
    fields: [
      ...
      { name: 'maxTextLength', type: 'integer', description: 'Maximum number of characters allowed.' },
      { name: 'cursorPosition', type: 'integer', description: 'Position of the cursor; defaults to end of text.' },
      { name: 'clearOnDownKey', type: 'boolean', description: 'When true, pressing Down erases all text.' },
      { name: 'active', type: 'boolean', description: 'When true, the cursor is visible.' },
      { name: 'secureMode', type: 'boolean', description: 'When true, characters are briefly shown then masked.' },
      ...
      { name: 'backgroundUri', type: 'uri', description: 'Background image URI.' },
      { name: 'leadingEllipsis', type: 'boolean', description: 'When true, overflow shows the end of text with a leading ellipsis.' },
    ],
    methods: [],
  },
  ...
  Keyboard: {
    ...
    fields: [
      ...
      { name: 'keyColor', type: 'string', description: 'Color for key labels and icons when keyboard lacks focus.' },
      { name: 'focusedKeyColor', type: 'string', description: 'Color for key labels and icons when keyboard has focus.' },
      { name: 'keyboardBitmapUri', type: 'uri', description: 'Custom keyboard background image.' },
      { name: 'focusBitmapUri', type: 'uri', description: 'Custom focus indicator image for the keyboard.' },
      { name: 'showTextEditBox', type: 'boolean', description: 'When true, the internal TextEditBox is visible above the keyboard.' },
      { name: 'textEditBox', type: 'node', description: 'The internal TextEditBox node.', readonly: true },
    ],
    methods: [],
  },
  ...
  PinPad: {
    ...
    fields: [
      { name: 'pin', type: 'string', description: 'The digits entered so far.' },
      { name: 'pinLength', type: 'integer', description: 'Maximum number of digits allowed.' },
      { name: 'secureMode', type: 'boolean', description: 'When true, digits are briefly shown then masked.' },
      { name: 'keyColor', type: 'string', description: 'Color for key labels and icons when unfocused.' },
      { name: 'focusedKeyColor', type: 'string', description: 'Color for key labels and icons when focused.' },
      { name: 'pinDisplayTextColor', type: 'string', description: 'Color of numbers shown in the PIN display boxes.' },
      { name: 'keyboardBitmapUri', type: 'uri', description: 'Custom keyboard background image.' },
      { name: 'pinDisplayBitmapUri', type: 'uri', description: 'Custom 9-patch image for the digit display boxes.' },
      { name: 'focusBitmapUri', type: 'uri', description: 'Custom 9-patch focus indicator image.' },
      { name: 'showPinDisplay', type: 'boolean', description: 'When true, the PIN display boxes are visible.' },
      { name: 'itemFocused', type: 'integer', description: 'Initial focus index when PinPad is created.' },
      { name: 'focusVisible', type: 'boolean', description: 'Controls whether the focused key appears highlighted on creation.' },
    ],
    methods: [],
  },
  ...
  Dialog: {
    ...
    fields: [
      ...
      { name: 'titleFont', type: 'node', description: 'Font for the title text.' },
      { name: 'message', type: 'string', description: 'Dialog message text.' },
      { name: 'messageColor', type: 'string', description: 'Message text color.' },
      { name: 'messageFont', type: 'node', description: 'Font for the message text.' },
      { name: 'numberedBullets', type: 'boolean', description: 'When true, bullets are shown as numbers.' },
      { name: 'bulletText', type: 'array', description: 'Array of bullet text strings.' },
      { name: 'bulletTextColor', type: 'string', description: 'Bullet text color.' },
      { name: 'bulletTextFont', type: 'node', description: 'Font for bullet text.' },
      ...
      { name: 'buttonSelected', type: 'integer', description: 'Index of the selected button.', readonly: true },
      { name: 'buttonFocused', type: 'integer', description: 'Index of the focused button.', readonly: true },
      { name: 'focusButton', type: 'integer', description: 'Write an index to move focus to that button.' },
      { name: 'backgroundUri', type: 'uri', description: 'Background image URI.' },
      { name: 'iconUri', type: 'uri', description: 'Small icon displayed next to the title.' },
      { name: 'dividerUri', type: 'uri', description: 'Divider bitmap between title and content.' },
      { name: 'optionsDialog', type: 'boolean', description: 'When true, the dialog auto-dismisses when Options is pressed.' },
      ...
      { name: 'maxHeight', type: 'float', description: 'Maximum dialog height before content scrolls.' },
    ],
    methods: [],
  },
  ...
  Panel: {
    ...
    fields: [
      ...
      { name: 'overhangTitle', type: 'string', description: 'Text shown in the overhang when this panel is left-positioned.' },
      { name: 'clockText', type: 'string', description: 'Text replacing the clock display in the overhang when this panel is active.' },
      { name: 'optionsAvailable', type: 'boolean', description: 'Enables the options button for this panel.' },
      { name: 'leftOrientation', type: 'boolean', description: 'True when the panel is in the left position, false when right.' },
      { name: 'leftOnly', type: 'boolean', description: 'When true, prevents the panel from appearing in the right position.' },
      { name: 'hasNextPanel', type: 'boolean', description: 'Signals that another panel exists to the right.' },
      { name: 'isFullScreen', type: 'boolean', description: 'When true, the panel occupies both left and right positions.' },
      { name: 'goBackCount', type: 'integer', description: 'Number of panels to navigate back on Back key press. Default 1.' },
      { name: 'selectButtonMovesPanelForward', type: 'boolean', description: 'When true, OK moves to the next panel.' },
      { name: 'isOffscreenLeft', type: 'boolean', description: 'Set by PanelSet when the panel is positioned off-screen to the left.' },
    ],
    methods: [],
  },
  ...
  Overhang: {
    ...
    fields: [
      { name: 'title', type: 'string', description: 'Text displayed on the left side of the overhang.' },
      { name: 'titleColor', type: 'string', description: 'Title text color.' },
      { name: 'logoUri', type: 'uri', description: 'Logo bitmap URI.' },
      { name: 'logoBaselineOffset', type: 'float', description: 'Vertical adjustment for logo alignment.' },
      { name: 'showClock', type: 'boolean', description: 'Whether the clock is shown on the right.' },
      ...
      { name: 'optionsAvailable', type: 'boolean', description: 'Whether the options key is currently functional.' },
      { name: 'optionsColor', type: 'string', description: 'Options indicator color when available.' },
      { name: 'optionsDimColor', type: 'string', description: 'Options indicator color when unavailable.' },
      { name: 'optionsIconColor', type: 'string', description: 'Tint for the options icon when available.' },
      { name: 'optionsIconDimColor', type: 'string', description: 'Tint for the options icon when unavailable.' },
      { name: 'optionsText', type: 'string', description: 'Text displayed next to the options symbol.' },
      { name: 'optionsMaxWidth', type: 'float', description: 'Maximum width for options text before ellipsis.' },
      { name: 'color', type: 'string', description: 'Solid background color.' },
      { name: 'backgroundUri', type: 'uri', description: 'Background image URI.' },
      { name: 'leftDividerUri', type: 'uri', description: 'Bitmap for the divider between logo and title.' },
      { name: 'leftDividerVertOffset', type: 'float', description: 'Vertical offset for the left divider.' },
      { name: 'rightDividerUri', type: 'uri', description: 'Bitmap for the divider between clock and options.' },
      { name: 'rightDividerVertOffset', type: 'float', description: 'Vertical offset for the right divider.' },
      { name: 'height', type: 'float', description: 'Height of the overhang region in pixels.' },
    ],
    methods: [],
  },
  ...
  Font: {
    ...
    fields: [
      ...
      { name: 'fallbackGlyph', type: 'string', description: 'Unicode character to display when an unsupported glyph is encountered.' },
    ],
    methods: [],
  },
};
