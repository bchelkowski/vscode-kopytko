...
exports.builtinArity = exports.keywordNames = exports.builtinNames = exports.BRIGHTSCRIPT_KEYWORDS = exports.BRIGHTSCRIPT_BUILTINS = void 0;
...
const _builtinMap = new Map(exports.BRIGHTSCRIPT_BUILTINS.map((b) => [b.name.toLowerCase(), b]));
function findBuiltin(name) {
    return _builtinMap.get(name.toLowerCase());
}
exports.BRIGHTSCRIPT_KEYWORDS = [
...
exports.builtinNames = new Set(exports.BRIGHTSCRIPT_BUILTINS.map((b) => b.name.toLowerCase()));
exports.keywordNames = new Set(exports.BRIGHTSCRIPT_KEYWORDS.map((k) => k.toLowerCase()));
/** Maps lowercased builtin name → { min, max } expected argument counts. */
exports.builtinArity = (() => {
    const map = new Map();
    for (const b of exports.BRIGHTSCRIPT_BUILTINS) {
        const m = /\(([^)]*)\)/.exec(b.signature);
        if (!m) {
            map.set(b.name.toLowerCase(), { min: 0, max: 0 });
            continue;
        }
