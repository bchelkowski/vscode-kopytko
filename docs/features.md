...
| macOS single-elevation redirect — a persistent, self-terminating root helper applies/reverts the pf anchor and stays alive across capture on/off toggles *and* device/port switches (re-validates and rebuilds the rules at runtime), so a whole VS Code session needs at most one admin-password prompt; self-heals within ~15s if the extension host goes silent, hard-stops on panel dispose/deactivate | ✅ | [network-inspector.md](./network-inspector.md#macos-single-elevation-persistent-helper) |
