...
const trivia_js_1 = require("./trivia.js");
...
function tokenize(source) {
    ...
    function scanWhitespace() {
        ...
        return { kind: trivia_js_1.TriviaKind.Whitespace, text: source.slice(start, pos), pos: start, end: pos };
    }
    function scanLineBreak() {
        ...
        return { kind: trivia_js_1.TriviaKind.LineBreak, text: source.slice(start, pos), pos: start, end: pos };
    }
    function scanTickComment() {
        const start = pos;
        advance(); // consume '
        while (pos < len && source[pos] !== '\n' && source[pos] !== '\r') {
            advance();
        }
        return { kind: trivia_js_1.TriviaKind.Comment, text: source.slice(start, pos), pos: start, end: pos };
    }
    function scanRemComment() {
        const start = pos;
        ...
        return { kind: trivia_js_1.TriviaKind.RemComment, text: source.slice(start, pos), pos: start, end: pos };
    }
    /**
    ...
    function isRemComment() {
        ...
        if (after !== undefined && isAlphaNumeric(after))
            return false;
        return isStatementStart();
    }
    ...
    function scanString() {
        ...
        while (pos < len) {
            if (source[pos] === '"') {
                ...
                // End of string
                return makeToken(tokenKind_js_1.TokenKind.StringLiteral, start, startLine, startCol);
            }
            if (source[pos] === '\n' || source[pos] === '\r') {
    ...
    function scanNumber() {
        ...
        if (source[pos] === '&' && pos + 1 < len && (source[pos + 1] === 'H' || source[pos + 1] === 'h')) {
            advance();
            advance(); // &H
            while (pos < len && isHexDigit(source[pos]))
                advance();
            // Trailing & for LongInteger
            if (pos < len && source[pos] === '&') {
                advance();
                return makeToken(tokenKind_js_1.TokenKind.LongIntegerLiteral, start, startLine, startCol);
            }
            return makeToken(tokenKind_js_1.TokenKind.IntegerLiteral, start, startLine, startCol);
        }
        ...
        if (isDouble)
            return makeToken(tokenKind_js_1.TokenKind.DoubleLiteral, start, startLine, startCol);
        if (isFloat)
            return makeToken(tokenKind_js_1.TokenKind.FloatLiteral, start, startLine, startCol);
        return makeToken(tokenKind_js_1.TokenKind.IntegerLiteral, start, startLine, startCol);
    }
    /**
    ...
    function scanIdentifierOrKeyword() {
        ...
        if (pos < len && '$%!#&'.includes(source[pos])) {
            ...
            return makeToken(tokenKind_js_1.TokenKind.Identifier, start, startLine, startCol);
        }
        ...
        const kwKind = tokenKind_js_1.KEYWORD_MAP.get(lower);
        if (kwKind !== undefined) {
     ...
     */
    function tryCompoundKeyword(firstWord, start, startLine, startCol) {
        const savedPos = pos;
        ...
        while (pos < len && isWhitespace(source[pos]))
            advance();
        if (pos === gapStart)
            return null; // no whitespace — not compound
        ...
        const kwKind = tokenKind_js_1.KEYWORD_MAP.get(compound);
        if (kwKind !== undefined) {
            return makeToken(kwKind, start, startLine, startCol);
        }
    ...
    function scanPreprocessor() {
        ...
        const wordStart = pos;
        while (pos < len && isAlpha(source[pos]))
            advance();
        const word = source.slice(wordStart, pos).toLowerCase();
        if (word === 'if')
            return makeToken(tokenKind_js_1.TokenKind.HashIf, start, startLine, startCol);
        if (word === 'const')
            return makeToken(tokenKind_js_1.TokenKind.HashConst, start, startLine, startCol);
        if (word === 'error') {
            // #error consumes the rest of the line as its message
            while (pos < len && source[pos] !== '\n' && source[pos] !== '\r')
                advance();
            return makeToken(tokenKind_js_1.TokenKind.HashError, start, startLine, startCol);
        }
        if (word === 'else') {
            ...
            if (w2 === 'if')
                return makeToken(tokenKind_js_1.TokenKind.HashElseIf, start, startLine, startCol);
            // Plain #else — revert past whitespace + word
            pos = savedPos2;
            line = savedLine2;
            column = savedCol2;
            return makeToken(tokenKind_js_1.TokenKind.HashElse, start, startLine, startCol);
        }
        if (word === 'elseif')
            return makeToken(tokenKind_js_1.TokenKind.HashElseIf, start, startLine, startCol);
        if (word === 'end') {
            ...
            const w2 = source.slice(w2Start, pos).toLowerCase();
            if (w2 === 'if')
                return makeToken(tokenKind_js_1.TokenKind.HashEndIf, start, startLine, startCol);
            pos = savedPos2;
            line = savedLine2;
            column = savedCol2;
            return makeToken(tokenKind_js_1.TokenKind.Unknown, start, startLine, startCol);
        }
        if (word === 'endif')
            return makeToken(tokenKind_js_1.TokenKind.HashEndIf, start, startLine, startCol);
    ...
    function scanOperatorOrPunctuation() {
        const start = pos;
        const startLine = line;
        const startCol = column;
        const ch = source[pos];
        switch (ch) {
            case '(':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.LeftParen, start, startLine, startCol);
            case ')':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.RightParen, start, startLine, startCol);
            case '[':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.LeftBracket, start, startLine, startCol);
            case ']':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.RightBracket, start, startLine, startCol);
            case '{':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.LeftBrace, start, startLine, startCol);
            case '}':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.RightBrace, start, startLine, startCol);
            case ',':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Comma, start, startLine, startCol);
            case ':':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Colon, start, startLine, startCol);
            case ';':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Semicolon, start, startLine, startCol);
            case '@':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.At, start, startLine, startCol);
            case '^':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Caret, start, startLine, startCol);
            case '\\':
                advance();
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.BackslashEqual, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Backslash, start, startLine, startCol);
            case '.':
                // Could be a decimal number starting with .
                if (pos + 1 < len && isDigit(source[pos + 1]))
                    return scanNumber();
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Dot, start, startLine, startCol);
            case '+':
                advance();
                if (pos < len && source[pos] === '+') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.PlusPlus, start, startLine, startCol);
                }
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.PlusEqual, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Plus, start, startLine, startCol);
            case '-':
                advance();
                if (pos < len && source[pos] === '-') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.MinusMinus, start, startLine, startCol);
                }
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.MinusEqual, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Minus, start, startLine, startCol);
            case '*':
                advance();
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.StarEqual, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Star, start, startLine, startCol);
            case '/':
                advance();
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.SlashEqual, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Slash, start, startLine, startCol);
            case '=':
                advance();
                return makeToken(tokenKind_js_1.TokenKind.Equal, start, startLine, startCol);
            case '<':
                advance();
                if (pos < len && source[pos] === '>') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.LessGreater, start, startLine, startCol);
                }
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.LessEqual, start, startLine, startCol);
                }
                if (pos < len && source[pos] === '<') {
                    advance();
                    if (pos < len && source[pos] === '=') {
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.LeftShiftEqual, start, startLine, startCol);
                    }
                    return makeToken(tokenKind_js_1.TokenKind.LeftShift, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Less, start, startLine, startCol);
            case '>':
                advance();
                if (pos < len && source[pos] === '=') {
                    advance();
                    return makeToken(tokenKind_js_1.TokenKind.GreaterEqual, start, startLine, startCol);
                }
                if (pos < len && source[pos] === '>') {
                    advance();
                    if (pos < len && source[pos] === '=') {
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.RightShiftEqual, start, startLine, startCol);
                    }
                    return makeToken(tokenKind_js_1.TokenKind.RightShift, start, startLine, startCol);
                }
                return makeToken(tokenKind_js_1.TokenKind.Greater, start, startLine, startCol);
            case '?':
                // Optional chaining: ?. ?[ ?( ?@
                if (pos + 1 < len) {
                    ...
                    if (next === '.') {
                        advance();
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.QuestionDot, start, startLine, startCol);
                    }
                    if (next === '[') {
                        advance();
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.QuestionBracket, start, startLine, startCol);
                    }
                    if (next === '(') {
                        advance();
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.QuestionParen, start, startLine, startCol);
                    }
                    if (next === '@') {
                        advance();
                        advance();
                        return makeToken(tokenKind_js_1.TokenKind.QuestionAt, start, startLine, startCol);
                    }
                }
                // Standalone ? is print shorthand
                advance();
                return makeToken(tokenKind_js_1.TokenKind.QuestionMark, start, startLine, startCol);
            default:
    ...
    // ── Token construction ────────────────────────────────────────────────
    function makeToken(kind, start, startLine, startCol) {
        return {
            kind,
    ...
    while (pos < len) {
        ...
        if (pos >= len) {
            ...
            tokens.push({
                kind: tokenKind_js_1.TokenKind.Eof,
                text: '',
                pos,
                end: pos,
                line,
