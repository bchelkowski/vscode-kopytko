...
/**
 * ⚠️ MIRROR of `kopytko-linter`'s `src/analysis/duplicateComponents.ts`.
 *
 * The canonical implementation lives in the linter, where the CLI project pass
 * uses it (`component/duplicate-name`). The extension cannot import it yet: it
 * consumes `kopytko-linter` as a published tarball (`^1.6.9`), not as a local
 * source, so the symbols do not exist in `node_modules` until the package is
 * released.
 *
 * **When `kopytko-linter` >= 1.7.0 is published and this repo's dependency is
 * bumped, delete this file** and change the import in
 * `services/componentDiagnostics.ts` to `from 'kopytko-linter'` — the exported
 * names and signatures are identical, so nothing else changes.
 *
 * Keep the two byte-identical until then. `LintDiagnostic`/`LintSeverity` are
 * already imported from the published package, so the shape cannot drift.
 */
